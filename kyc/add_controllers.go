package kyc

import (
	currencybase "github.com/ProtoconNet/mitum-currency/v3/base"
	"github.com/ProtoconNet/mitum2/base"
	"github.com/ProtoconNet/mitum2/util"
	"github.com/ProtoconNet/mitum2/util/hint"
	"github.com/ProtoconNet/mitum2/util/valuehash"
)

var (
	AddControllersFactHint = hint.MustNewHint("mitum-kyc-add-controllers-operation-fact-v0.0.1")
	AddControllersHint     = hint.MustNewHint("mitum-kyc-add-controllers-operation-v0.0.1")
)

var MaxAddControllersItems uint = 10

type AddControllersFact struct {
	base.BaseFact
	sender base.Address
	items  []AddControllersItem
}

func NewAddControllersFact(token []byte, sender base.Address, items []AddControllersItem) AddControllersFact {
	bf := base.NewBaseFact(AddControllersFactHint, token)
	fact := AddControllersFact{
		BaseFact: bf,
		sender:   sender,
		items:    items,
	}
	fact.SetHash(fact.GenerateHash())

	return fact
}

func (fact AddControllersFact) Hash() util.Hash {
	return fact.BaseFact.Hash()
}

func (fact AddControllersFact) GenerateHash() util.Hash {
	return valuehash.NewSHA256(fact.Bytes())
}

func (fact AddControllersFact) Bytes() []byte {
	is := make([][]byte, len(fact.items))
	for i := range fact.items {
		is[i] = fact.items[i].Bytes()
	}

	return util.ConcatBytesSlice(
		fact.Token(),
		fact.sender.Bytes(),
		util.ConcatBytesSlice(is...),
	)
}

func (fact AddControllersFact) IsValid(b []byte) error {
	if err := fact.BaseHinter.IsValid(nil); err != nil {
		return err
	}

	if err := currencybase.IsValidOperationFact(fact, b); err != nil {
		return err
	}

	if n := len(fact.items); n < 1 {
		return util.ErrInvalid.Errorf("empty items")
	} else if n > int(MaxAddControllersItems) {
		return util.ErrInvalid.Errorf("items, %d over max, %d", n, MaxAddControllersItems)
	}

	if err := fact.sender.IsValid(nil); err != nil {
		return err
	}

	founds := map[string]struct{}{}
	for _, it := range fact.items {
		if err := it.IsValid(nil); err != nil {
			return err
		}

		if it.contract.Equal(fact.sender) {
			return util.ErrInvalid.Errorf("contract address is same with sender, %q", fact.sender)
		}

		if _, found := founds[it.Controller().String()]; found {
			return util.ErrInvalid.Errorf("duplicate controller found, %s", it.Controller())
		}

		founds[it.controller.String()] = struct{}{}
	}

	return nil
}

func (fact AddControllersFact) Token() base.Token {
	return fact.BaseFact.Token()
}

func (fact AddControllersFact) Sender() base.Address {
	return fact.sender
}

func (fact AddControllersFact) Items() []AddControllersItem {
	return fact.items
}

func (fact AddControllersFact) Addresses() ([]base.Address, error) {
	as := []base.Address{}

	adrMap := make(map[string]struct{})
	for i := range fact.items {
		for j := range fact.items[i].Addresses() {
			if _, found := adrMap[fact.items[i].Addresses()[j].String()]; !found {
				adrMap[fact.items[i].Addresses()[j].String()] = struct{}{}
				as = append(as, fact.items[i].Addresses()[j])
			}
		}
	}
	as = append(as, fact.sender)

	return as, nil
}

type AddControllers struct {
	currencybase.BaseOperation
}

func NewAddControllers(fact AddControllersFact) (AddControllers, error) {
	return AddControllers{BaseOperation: currencybase.NewBaseOperation(AddControllersHint, fact)}, nil
}

func (op *AddControllers) HashSign(priv base.Privatekey, networkID base.NetworkID) error {
	err := op.Sign(priv, networkID)
	if err != nil {
		return err
	}
	return nil
}
